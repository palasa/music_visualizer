 ...
 *@param successCallBack function,decode成功后的回调函数，将arraybuffer作为this
*/
MusicVisualizer.load = function(xhr, path, successCallBack){
	xhr.abort();
	xhr.open('GET', path, true);
	xhr.responseType = 'arraybuffer';

	xhr.onload = function(){
		MusicVisualizer.isFunction(successCallBack) && successCallBack.call(xhr.response);
	}
	xhr.send();
}
...
MusicVisualizer.prototype.decode = function(arraybuffer, successCallBack, errorCallBack){
	var self = this;
	MusicVisualizer.ac.decodeAudioData(arraybuffer, function(buffer){
		var bufferSourceNode = MusicVisualizer.ac.createBufferSource();
		bufferSourceNode.buffer = buffer;
		successCallBack.call(bufferSourceNode);
	},function(err){
		errorCallBack(err);
	})
}
...
MusicVisualizer.prototype.addinit = function(callBack){
